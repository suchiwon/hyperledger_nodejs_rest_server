...
func (cc *PowerTradeChaincode) Init(stub shim.ChaincodeStubInterface) pb.Response {
	...
	err = stub.PutState(TRADE_SELL_COUNT, []byte(strconv.Itoa(cc.sellTradeCount)))

	if err != nil {
		return shim.Error(err.Error())
	}

	err = stub.PutState(TRADE_BUY_COUNT, []byte(strconv.Itoa(cc.buyTradeCount)))
...
func (cc *PowerTradeChaincode) publish(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if tradeType == TRADE_BUY_TYPE {
		...
		count, err = stub.GetState(TRADE_BUY_COUNT)

		if err != nil {
			return shim.Error(err.Error())
		}

		cc.buyTradeCount, _ = strconv.Atoi(string(count))

		tradeId = "BUY_" + string(count)
		cc.buyTradeCount++
		...
		count, err = stub.GetState(TRADE_SELL_COUNT)

		if err != nil {
			return shim.Error(err.Error())
		}

		cc.sellTradeCount, _ = strconv.Atoi(string(count))

		tradeId = "SELL_" + string(count)
		cc.sellTradeCount++
