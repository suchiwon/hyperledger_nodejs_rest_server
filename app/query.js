...
var queryChaincode = async function(peer, channelName, chaincodeName, args, fcn, username, org_name, responseCode) {
	try {
		...
		if(!channel) {
			...
			error_code = responseCode.ERROR_CODE.WAS_NO_CHANNEL.value;
		...
		if (response_payloads) {
			...
			return responseCode.makeFailureContractResponse(responseCode.ERROR_CODE.WAS_BAD_PROPOSAL.value, 'response_payloads is null');
		}
	} catch(error) {
		logger.error('Failed to query due to error: ' + error.stack ? error.stack : error);
		return responseCode.makeFailureContractResponse(responseCode.ERROR_CODE.WAS_BAD_PROPOSAL.value, error.toString());
	}
};
...
var getChannelPeers = async function(channelName, username, org_name) {
	try {
		// first setup the client for this org
		var client = await helper.getClientForOrg(org_name, username);
		logger.debug('Successfully got the fabric client for the organization "%s"', org_name);
		...
		let response_payload = await channel.getPeers();
		if (response_payload) {
			logger.debug(response_payload);
			return response_payload;
		} else {
