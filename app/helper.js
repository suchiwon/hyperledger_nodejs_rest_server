...
async function getClientForOrg (userorg, username) {
	...
	let inited = await client.initCredentialStores();

	if (!inited) {
		logger.debug("init CredentialStores failure");
	}
...
var getRegisteredUser = async function(username, userOrg, isJson) {
	try {
		...
		if (user && user.isEnrolled()) {
			...
			logger.info("admin info: username = %s, secret = %s", admins[0].username, admins[0].secret );
			let adminUserObj = await client.setUserContext({username: admins[0].username, password: admins[0].secret});
			logger.info("success set User Context admin");
			let caClient = client.getCertificateAuthority();
			logger.info("success get Certificate Autority");
